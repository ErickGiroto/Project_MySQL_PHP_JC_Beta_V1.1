...
if (isset($_GET['num_pedido'])) {
    ...
    $stmt = $conn->prepare("SELECT num_pedido, razao_social, cnpj, produto, tipo, tipo_alimento, qtd, status, data_registro, nome_usuario, data_de_entrega FROM pedidos_comercial WHERE num_pedido = ?");
    ...
    if ($pedidos && $empresa) {
        ...
        $pdf->SetTitle('Ordem de Servico #' . $num_pedido);
        ...
        <h1>Ordem de Serviço #{$num_pedido}</h1>
